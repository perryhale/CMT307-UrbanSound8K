...
min_keys = ['min_accuracy', 'min_precision', 'min_recall', 'min_f1']
mean_keys = ['mean_accuracy', 'mean_precision', 'mean_recall', 'mean_f1']
max_keys = ['max_accuracy', 'max_precision', 'max_recall', 'max_f1']

min_values = [np.min(x) for x in [accuracy, precision, recall, f1]]
mean_values = [np.mean(x) for x in [accuracy, precision, recall, f1]]
max_values = [np.max(x) for x in [accuracy, precision, recall, f1]]

dictionary_keys = [*min_keys, *mean_keys, *max_keys]
dictionary_values = [*min_values, *mean_values, *max_values]

dictionary = pd.DataFrame({k:[v] for k,v in zip(dictionary_keys, dictionary_values)})
dictionary.to_csv('kfold_min_mean_max_scores.csv', index=False)
...
dictionary = dictionary.set_index('fold')
dictionary = dictionary.to_csv('kfold_all_scores.csv')
